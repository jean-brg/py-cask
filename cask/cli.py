...
def validate_toml(config: dict) -> None:
    ...
    if not os.path.exists(app["entry"]):
        ...
        sys.exit(1)
    if app.get("icon") and not os.path.exists(app["icon"]):
        print(f"Error: icon file '{app['icon']}' not found")
        sys.exit(1)

    for source in config.get("data", {}):
        if not os.path.exists(source):
            print(f"Error: data source '{source}' not found")
            sys.exit(1)
...
def _toml_character_escape(value: str) -> str:
...
def run_pyinstaller_cmd(cmd: list[str], verbose: bool) -> tuple[bool, int]:
    ...
    for line in process.stdout:
        if verbose:
            ...
            parts = line.split(" ", 1)
            line = parts[1].rstrip() if len(parts) > 1 else line.rstrip()
...
def cmd_build(args: argparse.Namespace) -> None:
    ...
    if not args.keep:
        try:
            ...
            print(f"Warning: could not fully clean up build artifacts: {e}")
...
def generate_toml(app_name: str, entry: str, icon: str, data: list[str]) -> str:
    ...
    lines = [
        ...
        "[app]",
        f'name = "{_toml_character_escape(app_name)}"',
        f'entry = "{_toml_character_escape(entry)}"',
    ]
    if icon:
        lines.append(f'icon = "{_toml_character_escape(icon)}"')
    lines.append("")

    if data:
        lines.append("[data]")
        for folder in data:
            lines.append(f'"{_toml_character_escape(folder)}" = "{_toml_character_escape(folder)}"')
        lines.append("")
